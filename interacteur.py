...
class Interacteur:
    ...
    def yesNoQuestion(self, message, yesParDefaut=True):
        yesParDefaut = True if yesParDefaut == True else False
        avertisseur = " (Y/n)" if yesParDefaut else " (y/N)" 
        print(message + avertisseur)
        choix = input()
        if choix.upper() == "Y":
            return True
        elif choix.upper() == "N":
